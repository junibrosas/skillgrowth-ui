...
import { CourseOverviewComponent } from './course-overview.container';
...
describe('Component: CourseOverviewComponent', () => {
    ...
    it('should getCourse() call course service getByIdAndPublished() and call proper methods from success callback.', () => {
        ...
        expect(storeSpy.calls.first().args[0].payload[0].url).toEqual('/course/feed', 'wrong course url');
        expect(storeSpy.calls.first().args[0].payload[1].label).toEqual(defaultCourse.name, 'wrong course detail label');
        expect(storeSpy.calls.first().args[0].payload[1].url).toEqual('/course/detail/' + defaultCourse.id, 'wrong course detail url');
    ...
    it('should viewModule() have correct navigation url', () => {
        ...
        expect(routerSpy.calls.first().args[0][0]).toEqual('/module/detail/');
