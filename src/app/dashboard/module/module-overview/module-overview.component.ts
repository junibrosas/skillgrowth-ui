...
export class ModuleOverviewComponent implements OnDestroy, OnInit {
    ...
    getModule() {
        ...
        this.enrollService.getByUserOwned(this.moduleId, this.currentUser.id).subscribe(
            payload => {
                this.spinner.hide();
                ...
                this.store.dispatch({
                    type: COMMON_BREADCRUMB_SET, payload: [
                        ...
                        { label: payload.module.course.name, url: '/dashboard/course/detail/' + payload.module.course.id, params: [] },
                        { label: payload.module.name, url: '/dashboard/module/detail/' + payload.module.id, params: [] }
                    ]
                });
    ...
    markAsComplete(isComplete: boolean) {
        ...
        this.enrollService.markAsComplete(this.moduleId, this.courseId, this.currentUser.id, isComplete).subscribe(
            () => {
                this.spinner.hide();
